...
function generateRandomOrder(){
  ...
  return {
    ...
    items, status:statuses[getRandomInt(0,statuses.length-1)],
...
let orders=initialOrders.slice(), filteredOrders=orders.slice();
...
function renderOrders(){
  if(!filteredOrders.length){
    ordersTableBody.innerHTML = 
      '<tr><td colspan="8" class="text-center py-5">주문이 없습니다.</td></tr>';
    return;
  }
  ordersTableBody.innerHTML = filteredOrders.map(o=>{
    ...
    return `<tr>
      ...
      <td><button class="action-btn view-btn" onclick="openOrderModal('${o.id}')">
...
function exportCSV(){
  ...
  orders.forEach(o=>{
    const cnt=o.items.reduce((s,i)=>s+i.qty,0),
          totalAmt=o.items.reduce((s,i)=>s+i.qty*i.price,0);
    rows.push([o.id, `"${o.date}"`, `"${o.customer}"`, cnt, totalAmt, o.status].join(','));
  });
  const blob=new Blob([rows.join('\n')],{type:'text/csv;charset=utf-8;'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');
  a.href=url; a.download='orders_export.csv';
